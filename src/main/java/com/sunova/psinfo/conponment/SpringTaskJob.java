...
public class SpringTaskJob {
    ...
    public void get_dd_access_key(){
        ...
        int i = dingTalkService.update_access_key(Context.DD_Contacts_Appkey,Context.DD_Contacts_Appsecret);
        if(i!=1){
            logger.error("更新钉钉通讯录密钥失败，联系管理员查明原因！");
        }else {
            logger.info("******更新钉钉通讯录密钥成功******");
        }
        int j = dingTalkService.update_access_key(Context.DD_AD_Appkey,Context.DD_AD_Appsecret);
        if(j!=1){
